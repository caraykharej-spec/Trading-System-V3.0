...
class SQLitePendingOrderRepository(PendingOrderRepository):
        ...
        for row in rows:
            order_row = self.connection.execute(
                """SELECT order_id, symbol, side, order_type, quantity, requested_price,
                   stop_loss, take_profit, leverage, created_at FROM orders WHERE order_id = ?""",
                (row[0],),
            ).fetchone()
            if order_row is None:
                raise ValueError(f"Pending order references missing order: {row[0]}")
            order = OrderRequest(
                order_id=str(order_row[0]), symbol=str(order_row[1]), side=PositionSide(str(order_row[2])),
                order_type=OrderType(str(order_row[3])), quantity=Decimal(str(order_row[4])),
                requested_price=Decimal(str(order_row[5])) if order_row[5] is not None else None,
    ...
    def save(self, order: PendingOrder) -> None:
        ...
        self.connection.execute(
            ...
            (order.order.order_id, order.status.value, order.accepted_at.isoformat(),
             order.updated_at.isoformat(), order.cancel_reason, order.rejection_reason),
        )
        if self.auto_commit:
