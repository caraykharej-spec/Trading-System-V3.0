...
class SQLiteOrderRepository(OrderRepository):
    ...
    def save_request(self, order: OrderRequest) -> None:
        self.connection.execute(
            """INSERT INTO orders (
                ...
                stop_loss, take_profit, leverage, created_at, status, decision_snapshot
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                order.order_id,
                order.symbol,
    ...
    def save_result(self, result: OrderResult) -> None:
        ...
        self.connection.execute(
            """UPDATE orders
               SET status = ?, symbol = ?, filled_price = ?, reason = ?, filled_at = ?
               WHERE order_id = ?""",
            (
                result.status.value,
                result.symbol,
    ...
    def get(self, order_id: str) -> tuple[OrderRequest, OrderResult | None] | None:
        row = self.connection.execute(
            """SELECT order_id, symbol, side, order_type, quantity, requested_price,
               stop_loss, take_profit, leverage, created_at, status, filled_price, reason,
               filled_at, decision_snapshot
        ...
        request = OrderRequest(
            ...
            decision_snapshot=str(row[14]) if row[14] is not None else None,
        )
        result = (
            None
