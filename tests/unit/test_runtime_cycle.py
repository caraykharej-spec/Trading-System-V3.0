...
def make_position() -> Position:
    return Position("P-1", "BTC/USDT", PositionSide.LONG, Decimal("100"), Decimal("95"), Decimal("100"), Decimal("1"), Decimal("1"), Decimal("110"))


def make_order() -> OrderRequest:
    return OrderRequest("PAPER-1", "BTC/USDT", PositionSide.LONG, OrderType.MARKET, Decimal("1"), None, Decimal("95"), Decimal("110"), Decimal("1"))
...
def make_paper_runtime(price=Decimal("100")):
    ...
    manager = PendingOrderManager(pending, executor, lambda o, r: atomic.apply_fill(o, r, fill_id=f"fill:{r.order_id}"))
...
def test_runtime_cycle_persists_completion_and_stop_out() -> None:
    ...
    runner = RuntimeCycleOrchestrator(positions, lambda _: Decimal("94"), Account(Decimal("1000")), audits)
...
def test_failed_cycle_is_audited() -> None:
    audits = InMemoryCycleAuditRepository()
    runner = RuntimeCycleOrchestrator(InMemoryPositionRepository([make_position()]), lambda _: Decimal("0"), Account(Decimal("1000")), audits, exit_policy=ExitPolicy())
    try:
        runner.run("C-3")
    except ValueError:
        pass
...
def test_runtime_cycle_can_submit_selected_paper_order():
    ...
    runner = RuntimeCycleOrchestrator(
        positions, lambda _: Decimal("100"), Account(Decimal("1000")), audits,
        pending_order_manager=manager, paper_runtime=paper,
...
def test_runtime_cycle_pending_limit_is_checked_before_new_submission():
    ...
    order = OrderRequest("PENDING-1", "BTC/USDT", PositionSide.LONG, OrderType.LIMIT, Decimal("1"), Decimal("100"), Decimal("90"), Decimal("120"), Decimal("1"))
    paper.submit(order)
    assert positions.list_open() == []
    manager.executor._live_price_provider = lambda _: Decimal("99")
    audits = InMemoryCycleAuditRepository()
    runner = RuntimeCycleOrchestrator(positions, lambda _: Decimal("99"), Account(Decimal("1000")), audits, pending_order_manager=manager)
