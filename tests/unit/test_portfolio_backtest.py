...
def bar(symbol: str, tf: str, minute: int, close: str = "100") -> Candle:
    ...
    return Candle(symbol, tf, ts, Decimal("100"), Decimal("101"), Decimal("99"), Decimal(close), Decimal("100"))


def source(symbol: str) -> dict[str, list[Candle]]:
    return {tf: [bar(symbol, tf, 0), bar(symbol, tf, 15), bar(symbol, tf, 30)] for tf in ("1d", "4h", "1h", "15m")}


def test_portfolio_uses_shared_equity_and_records_curve() -> None:
    signal = SimpleNamespace(state=StrategyState.READY_FOR_RISK_REVIEW, direction="LONG", setup="TEST", stop_loss=Decimal("95"), target=Decimal("105"))
    ...
    with patch.object(engine._helpers, "_snapshots_at", return_value=snapshots), patch("app.backtest.portfolio.evaluate_strategy", return_value=signal):
