...
def test_stale_live_price_is_rejected():
    price = LivePrice(symbol="BTC/USDT", price=Decimal("70000"), as_of=datetime.now(timezone.utc) - timedelta(minutes=3), provider="test")
...
def test_invalid_candle_ohlc_is_rejected():
    candle = Candle(symbol="BTC/USDT", timeframe="1h", timestamp=datetime.now(timezone.utc), open=Decimal("90"), high=Decimal("80"), low=Decimal("100"), close=Decimal("95"), volume=Decimal("1"))
