...
def test_backtest_fills_next_bar_and_hits_take_profit() -> None:
    ...
    engine = BacktestEngine(
        BacktestConfig(initial_equity=Decimal("10000"), risk_per_trade_percent=Decimal("1"))
    )
    with (
        patch.object(
            engine,
            "_snapshots_at",
            return_value={"1d": object(), "4h": object(), "1h": object(), "15m": object()},
        ),
        patch("app.backtest.engine.evaluate_strategy", return_value=signal),
    ):
